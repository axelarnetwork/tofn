...
use crate::{
    collections::{FillP2ps, FillVecMap, P2ps, VecMap, XP2ps},
    sdk::{
...
pub trait Executer: Send + Sync {
    ...
    fn execute(
        ...
        bcasts_in: VecMap<Self::Index, Option<Self::Bcast>>,
...
impl<T: Executer> ExecuterRaw for T {
    ...
    fn execute_raw(
        ...
        let bcasts_deserialized: VecMap<_, Result<_, _>> =
            bcasts_in.map_to_vecmap(|bytes| bincode::deserialize(&bytes))?;
        ...
        let p2ps_in = p2ps_deserialized.xmap(Result::unwrap);
