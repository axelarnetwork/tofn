...
mod tests {
    ...
    fn basic_correctness() {
        ...
        let (a_zkp, _) = ZkSetup::new_unsafe(&mut rand::thread_rng(), TypedUsize::from_usize(0)).unwrap();
        let (b_zkp, _) = ZkSetup::new_unsafe(&mut rand::thread_rng(), TypedUsize::from_usize(1)).unwrap();
